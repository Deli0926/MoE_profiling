...
OUTPUT_DIR = "./output/ep_pp_tradeoff_saddlepoint"
...
def marginal_saddlepoint_distribution(pattern_pmf, local_bs, ep_size):
    max_load = local_bs * TOP_K
    marginal_pmfs = []

    for gpu_idx in range(ep_size):
        single_token_pmf = marginal_count_pmf(pattern_pmf, gpu_idx)
        saddle = SaddlepointMarginal(single_token_pmf, local_bs)
...
def plot_metric_by_ep(rows, metric, ylabel, out_path):
    ...
    plt.ylabel(ylabel)
    plt.title(f"{ylabel} across EP / PP choices (saddlepoint)")
...
def main():
    ...
    print(
        "global bs를 고정하고 EP / PP 조합별 saddlepoint 근사 max routed-token load를 계산 중...\n"
        f"(global bs={global_bs}, source ep={source_ep}, source bs={source_bs})"
    )
    print(
        "가정 1: PP를 적용하면 각 stage에서 한 번에 처리하는 micro-batch 크기는 ceil(global_bs / PP) 입니다.\n"
        "가정 2: 현재 구현은 GPU별 marginal saddlepoint + GPU 간 independence approximation 입니다."
    )
