# Formulas Used in `count_expert_tokens.py`

This note summarizes the mathematical steps used in `MoE_profiling/count_expert_tokens.py`.
...
## 2. Token-Level Expert Selection
...
## 3. Empirical Token Pattern PMF from Source Data

Suppose the source file contains $T$ tokens in total, and the GPU pattern for token $t$ is $X^{(t)}$.
...
This corresponds to the `pattern_pmf` dictionary in the code.

## 4. Batch-Level GPU Load Vector

To predict a larger batch of size $B$, the script assumes that token patterns are i.i.d. samples from $\hat{p}(x)$.
...
The code computes this convolution exactly using repeated sparse distribution convolution with binary exponentiation.

## 5. Max Routed Tokens per GPU
...
## 6. Expected Value and Percentile
...
## 9. Layer-Wise Averaging

If the model has $R$ valid layers and each layer yields a PMF $\mathbb{P}_{\ell}(Y=m)$, then the final reported average distribution is
